...
Elm.VideoVoting = Elm.VideoVoting || {};
Elm.VideoVoting.make = function (_elm) {
   "use strict";
   _elm.VideoVoting = _elm.VideoVoting || {};
   if (_elm.VideoVoting.values) return _elm.VideoVoting.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var VoteFor = function (a) {    return {ctor: "VoteFor",_0: a};};
   var Delete = function (a) {    return {ctor: "Delete",_0: a};};
   var videoDetail = F3(function (address,max,video) {
      return A2($Html.button,
      _U.list([$Html$Attributes.$class(A2(classForVideo,max,video.votes)),A2($Html$Events.onClick,address,VoteFor(video.id))]),
      _U.list([A2($Html.span,_U.list([$Html$Attributes.$class("badge")]),_U.list([$Html.text($Basics.toString(video.votes))]))
              ,$Html.text(video.uri)
              ,A2($Html.a,
              _U.list([$Html$Attributes.$class("delete"),A2($Html$Events.onClick,address,Delete(video.id)),$Html$Attributes.href("#")]),
              _U.list([$Html.text("×")]))]));
   });
   var videoList = F2(function (address,videos) {
      var max = A2($Maybe.withDefault,0,$List.maximum(A2($List.map,function (_) {    return _.votes;},videos)));
      return A2($Html.div,_U.list([$Html$Attributes.$class("list-group")]),A2($List.map,A2(videoDetail,address,max),videos));
   });
   ...
   var videoEntry = F2(function (address,url) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("form-inline")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("form-group")]),
              _U.list([A2($Html.label,_U.list([$Html$Attributes.$for("newVideo")]),_U.list([$Html.text("What do you want to watch?")]))
                      ,$Html.text(" ")
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([A2(videoEntry,address,model.field),A2(videoList,address,model.videos)]));
   });
   var NoOp = {ctor: "NoOp"};
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Add": return _U.update(model,
           {uid: model.uid + 1
           ,field: ""
           ,videos: $String.isEmpty(model.field) ? model.videos : A2($Basics._op["++"],
           model.videos,
           _U.list([A2(newVideo,A2($Basics._op["++"],"https://www.youtube.com/watch?v=",model.field),model.uid)]))});
         case "UpdateField": return _U.update(model,{field: _p2._0});
         case "Delete": return _U.update(model,{videos: A2($List.filter,function (v) {    return !_U.eq(v.id,_p2._0);},model.videos)});
